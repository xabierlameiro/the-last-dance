    ...
    const questions = [
        {
            ...
            questionHtml: `
                    <h1>Hola ${parsedName.charAt(0).toUpperCase() + parsedName.slice(1)
        ...
        if (state.currentQuestion === questions.length - 1 && !emailRef.current) {
            (async () => {
                try {
                        ...
                        body: JSON.stringify({
                            ...
                            message: `
                                    ...
                                    <ol> 
                                        ${state.answers.map(
                                (answer) =>
                                    `<li> ${answer.question} : ${answer.answer} - ${answer.isCorrect ? '✅' : '🚫'
                                    } </li>`
                            )}
                                    </ol>
                                    `.replace(/,/g, ''),
                    ...
                    console.error('Failed to send survey email:', e);
                }
            })();
        }
