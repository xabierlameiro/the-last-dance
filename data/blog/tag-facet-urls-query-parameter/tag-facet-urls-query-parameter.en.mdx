...
locale: 'en'
updated: '2026-09-23'
excerpt: '95 duplicate URLs, 229 impressions, 1 click · three fixes that broke navigation or could not work · the ?tag= design that can'
...
- `/blog/hydration/uncaught-error-minified-react-error` returns `308` to `/blog/error/uncaught-error-minified-react-error?tag=hydration`, which returns `200`. Same for the other legacy paths I sampled, in English and Spanish.
- `robots.txt` carries `Disallow: /*?tag=` in its `User-agent: *` group.
- Every internal blog link on the built pages uses a category segment.
