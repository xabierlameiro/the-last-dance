...
description: 'Tres fugas de memoria de Next.js 15.5–16.3, como identificar a túa con heap snapshots e por que en serverless a fuga se disfraza dun 504.'
...
faq:
    - question: 'Por que a memoria do meu servidor Next.js non para de medrar?'
      answer: 'Tres fugas no propio framework cubrían de Next.js 15.5 a 16.3: a caché LRU do router non contabiliza as súas chaves (issue 94890), a árbore RSC queda retida por request cando o cliente corta o stream (issue 94919) e os timeouts do middleware quedan retidos polo sandbox (issue 95094). As tres pecháronse como resoltas upstream entre o 25 e o 27 de xullo de 2026, así que actualizar é o primeiro paso — pero se segues nunha versión afectada, compara a curva de heap coas URLs únicas, o tráfico total e as rutas concretas antes de auditar o teu código: cada fuga ten unha forma distinta.'
    ...
    - question: 'Como comprobo se a miña app Next.js ten unha fuga de memoria?'
      answer: 'Compila con output standalone e executa npx next-leak sobre o directorio da app. Mide cada ruta nun proceso limpo con GC forzado antes de cada mostra, clasifica a curva post-GC como leak, stable ou inconclusive e nomea os obxectos que reteñen memoria — cos snapshots crus e un run.json para verificar o veredicto en Chrome DevTools.'
---
...
Se o teu servidor Next.js autoaloxado medra en memoria ata morrer con `FATAL ERROR: Reached heap limit Allocation failed - JavaScript heap out of memory` — ou o contedor cae con `OOMKilled` (exit code 137) — non empeces auditando o teu código. Hai tres fugas de memoria documentadas no propio framework, de Next.js 15.5 a 16.3, e son o primeiro que hai que descartar:
...
**Actualización, agosto de 2026: as tres pecháronse como resoltas upstream entre o 25 e o 27 de xullo de 2026**, días despois de publicarse este post. Iso non deixa obsoleto o que vén debaixo: segues a correr a versión que despregaches, e as tres formas son xusto o que distingue "estou nunha versión que aínda a ten" de "a fuga é miña". Mira nos issues ligados que release leva cada arranxo, e actualiza antes de auditar o teu propio código. O [#84884](https://github.com/vercel/next.js/issues/84884) (axios con `AbortSignal` en middleware) segue aberto a día de hoxe.
...
## Como o medín (en vez de fiarme das miñas intuicións)

Todo o anterior saíu de comparar snapshots á man. Á terceira vez de repetir ese ritual — warm-up, GC forzado, snapshot base, carga, repouso, GC outra vez, mostra, diff — automaticeino nun CLI: [next-leak](https://github.com/xabierlameiro/next-leak).

```bash
npx next-leak . --quick
```

Arrinca o teu build standalone ruta a ruta nun proceso limpo, xera carga real (incluídos clientes que colgan a metade do stream), forza GC antes de cada mostra e xulga a **forma** da curva post-GC, non os tamaños absolutos. Contra a reprodución de #95094 atopou a fuga e nomeou o retedor pola súa conta:
