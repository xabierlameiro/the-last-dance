...
description: 'Three memory leaks in Next.js 15.5–16.3, how to tell which one you have from heap snapshots, and why on serverless a leak shows up as a 504 timeout.'
...
faq:
    - question: 'Why does my Next.js server keep growing in memory?'
      answer: 'Three leaks in the framework itself covered Next.js 15.5 to 16.3: the router LRU cache does not count its keys (issue 94890), the RSC render tree is retained per request when the client aborts the stream (issue 94919), and middleware timeouts are retained by the sandbox (issue 95094). All three were closed as completed upstream between 25 and 27 July 2026, so upgrading is the first move — but if you are still on an affected version, compare the heap curve against unique URLs, total traffic and specific routes before auditing your own code: each leak has a distinct shape.'
...
**Update, August 2026: all three were closed as completed upstream between 25 and 27 July 2026**, days after this post first went out. That does not make the section below obsolete — you are still running whatever version you deployed, and the three shapes are exactly how you tell "I'm on a version that still has it" from "the leak is mine". Check the linked issues for the release that carries each fix, then upgrade before auditing your own code. [#84884](https://github.com/vercel/next.js/issues/84884) (axios with `AbortSignal` in middleware) is still open at the time of writing.
