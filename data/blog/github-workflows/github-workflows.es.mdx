...
<Image
    ...
    alt="Diagrama del pipeline de CI/CD: desde la rama dev se ejecutan en paralelo la cancelación de redundancia, la revisión de código, los tests unitarios y de extremo a extremo, y la generación de documentación con JSDoc y Storybook, publicando cada uno su informe; el versionado dispara después el despliegue al entorno de preproducción. Una pull request a la rama master dispara el despliegue a producción, tras el cual se ejecuta el test de rendimiento de Lighthouse y se publica su informe."
    width="1024"
    height="726"
    sizes="100vw"
...
## Cinco cosas que merece la pena hacer desde el día uno
...
- **Cancela runs redundantes con `concurrency:`, no con una action.** Hacer push tres veces seguidas no debería lanzar tres pipelines enteros. Aquí había un paso de `styfle/cancel-workflow-action`, y lo quité de mi propio pipeline por dos motivos. Estaba fijado con un **tag mutable**, que es justo lo que la [guía de hardening de GitHub](https://docs.github.com/en/actions/reference/security/secure-use) te dice que no hagas con actions de terceros — un tag se puede reapuntar a código nuevo, que es exactamente como el compromiso de `tj-actions/changed-files` llegó a miles de repositorios en marzo de 2025. Y necesitaba `access_token`, así que en cada push le entregaba a un tercero un PAT con permisos de escritura sobre otros cinco repositorios. La clave nativa `concurrency:` hace lo mismo sin token y sin código ajeno.
- **Define `permissions:` explícitamente.** Sin ella, el `GITHUB_TOKEN` se lleva el valor por defecto del repositorio, que es más amplio de lo que un job de linting necesita. `contents: read` arriba del fichero, y subirlo por job donde algo tenga que escribir de verdad, es todo el cambio.
- **Mantén al día los majors de las actions.** `actions/checkout@v4` y `setup-node@v4` siguen funcionando, pero declaran el runtime de Node 20, que GitHub lleva tiempo retirando de los runners. El major actual de ambas es `@v7`, y corre sobre Node 24.
