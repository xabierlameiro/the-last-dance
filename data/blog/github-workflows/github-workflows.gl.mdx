...
<Image
    ...
    alt="Diagrama do pipeline de CI/CD: desde a rama dev execútanse en paralelo a cancelación de redundancia, a revisión de código, os tests unitarios e de extremo a extremo, e a xeración de documentación con JSDoc e Storybook, publicando cada un o seu informe; o versionado dispara despois o despregamento ao contorno de preprodución. Unha pull request á rama master dispara o despregamento a produción, tras o cal se executa o test de rendemento de Lighthouse e publícase o seu informe."
    width="1024"
    height="726"
    sizes="100vw"
...
- **Cancela runs redundantes con `concurrency:`, non cunha action.** Facer push tres veces seguidas non debería lanzar tres pipelines enteiros. Aquí había un paso de `styfle/cancel-workflow-action`, e quiteino do meu propio pipeline por dous motivos. Estaba fixado cun **tag mutable**, que é xusto o que a [guía de hardening de GitHub](https://docs.github.com/en/actions/reference/security/secure-use) che di que non fagas con actions de terceiros — un tag pódese reapuntar a código novo, que é exactamente como o compromiso de `tj-actions/changed-files` chegou a milleiros de repositorios en marzo de 2025. E precisaba `access_token`, así que en cada push entregáballe a un terceiro un PAT con permisos de escritura sobre outros cinco repositorios. A clave nativa `concurrency:` fai o mesmo sen token e sen código alleo.
- **Define `permissions:` explicitamente.** Sen ela, o `GITHUB_TOKEN` leva o valor por defecto do repositorio, que é máis amplo do que un job de linting precisa. `contents: read` arriba do ficheiro, e subilo por job onde algo teña que escribir de verdade, é todo o cambio.
- **Mantén ao día os majors das actions.** `actions/checkout@v4` e `setup-node@v4` seguen funcionando, pero declaran o runtime de Node 20, que GitHub leva tempo retirando dos runners. O major actual de ambas é `@v7`, e corre sobre Node 24.
